 ...
 */
export function AgentClientSelect() {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      ...
      try {
        ...
        console.error('获取智能体客户端数据失败:', error);
        setError(true);
        setClientOptions([]);
      } finally {
        setLoading(false);
            ...
            {({ field: clientIdField }) => {
              ...
              if (clientIdField.value && typeof clientIdField.value === 'string') {
                ...
                if (typeof nodeInputsValues.clientId === 'string' && nodeInputsValues.clientId) {
                  ...
                  typeof nodeInputsValues.clientName === 'string' && nodeInputsValues.clientName
                ) {
              ...
              return (
                <FormItem name="智能体客户端" type="string" required={true} labelWidth={100}>
                  <Select
                    placeholder={loading ? '加载中...' : '请选择智能体客户端'}
                    style={{ width: '100%' }}
                    value={displayValue}
                    onChange={(value) => {
